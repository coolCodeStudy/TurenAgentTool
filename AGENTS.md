...
- Do not make the Global Project Manager the default watcher for feature-level child threads. Feature-level user decisions should return to the Feature Coordinator, which asks the user and then continues that feature flow. The Global PM should audit portfolio health, stale coordinators, missing watch paths, cross-feature conflicts, merge/deploy permission, and system/process issues; it should only recover a feature flow when the feature coordinator is idle, blocked, or missing its watch path.
