...
- Use `docs/product/Acceptance-Testing-Agent-Protocol.md` as the Quality & Acceptance Lead protocol for risk-based quality routing and user-facing acceptance testing.
- Track operating-model infrastructure in `docs/project-management/Agent-Operating-Model-Roadmap.md`. Do not put internal agent-system improvements in `Feature-Registry.md` unless they create a user-facing product feature.
- Use `docs/project-management/Coordinator-Context-Packet.md` when creating, resuming, taking over, or recovering a Feature Coordinator with existing delivery history.
- Use prompt templates under `docs/project-management/prompt-templates/` for repeatable Feature Coordinator, Release Integration, Quality & Acceptance, Deploy Conflict Resolution, and Frontend Review dispatches.
...
- For Global Project Manager portfolio-health checks, run `python3 scripts/audit_agent_flow_health.py` before reading individual coordinator or child-agent conversations. Use conversation context only when the audit marks `context_required: yes` or when repo-native state is contradictory or insufficient.
- After changing Agent Operating Model, Delivery Coordinator rules, flow-health audits, or prompt templates, run `python3 scripts/evaluate_agent_flow_cases.py` to verify known multi-agent failure patterns are still detected.
...
- Feature Coordinators should not report every routine child-role transition to the Global Project Manager or Owner. They should continue autonomously through normal Product, Development, deploy, and Acceptance handoffs. Report upward only for milestone summaries, true blockers, user/global decisions, cross-feature conflicts, credentials/permissions, or readiness for user acceptance.
- A Feature Coordinator is the directly responsible owner for one feature until it reaches a terminal state: `product_done`, `blocked_with_owner`, `waiting_for_user_acceptance`, or `explicitly_cancelled`. A child role returning, pushing a branch, or recommending the next owner does not transfer ownership to the Global PM.
- Routine Product -> Development -> Review -> Deploy -> Acceptance -> Retest routing must stay inside the feature coordinator loop. `Return to Global PM` is invalid for normal next-role routing unless the row names a true global trigger such as cross-feature release conflict, stale coordinator recovery, credentials/permissions, user priority decision, or operating-model defect.
- Each coordinator turn after a child return must end in one of four closure states: `accept_and_route`, `reject_and_return`, `blocked_with_owner`, or `ready_for_user_acceptance`. Anything else is unfinished coordination.
...
- If a Feature Coordinator reaches a terminal state on a non-authoritative branch or worktree, run a state reconciliation gate before portfolio status is reported: compare the coordinator branch against the authoritative branch for `Feature-Registry.md`, `Acceptance-Queue.md`, `Delivery-Queue.md`, and any linked PRD/tech plan; cherry-pick/manual-port only the valid state, or record a precise reconciliation blocker. Use `python3 scripts/audit_agent_flow_health.py --compare-ref <coordinator-ref> --feature "<feature>"` when a coordinator branch/ref is known.
- Apply the anti-rationalization gate before stopping any substantial coordinator flow: `branch pushed`, `after deploy`, `later`, `I will wait`, `Coordinator/Ops`, `someone`, and `Return to Global PM` are not valid closure unless the coordinator has also recorded a concrete owner, event, ref, deploy decision, or blocker.
- For cloud-served or browser-tested features, a returned "code fixed and pushed" result must become an explicit deploy/retest next step. The coordinator must name or dispatch the deploy owner before routing to Quality & Acceptance, and must not stop at "after deploy, retest" without an owner, branch or commit, deploy path, affected URL/service, and watch path.
...
- Governance-only changes may be pushed to `main` after verification when they match the GitHub Actions `no_deploy` classification in `docs/project-management/Deploy-Classification.md`. The executable classifier is `scripts/classify_deploy_change.py`, and its regression tests are in `tests/test_deploy_change_classifier.py`. Do not hold these changes on feature branches only because of deploy fear; the workflow should pass the `no_deploy` job without restarting production.
