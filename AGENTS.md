...
- After a task branch with product-affecting changes is pushed, do not stop at "pushed" by default. Product-affecting changes include code, configuration, schema, frontend assets, service behavior, or runtime data-flow changes. Unless the user explicitly asks to pause, continue the release path by merging the pushed branch into the appropriate integration/release branch and deploying the resulting pushed ref. State the exact merge target and deploy path before acting, and still obey remote credential and cloud-service approval boundaries. Pure documentation, tech plan, PRD, or comment-only changes should be committed and pushed, but should not be deployed unless those files are served by the product or the user explicitly asks for deployment.
...
- After a GitHub push for product-affecting changes is explicitly requested or approved, treat remote deployment through the standard Ops API path as pre-approved for that pushed ref unless the user says to pause. State the exact ref, deploy mode, and verification URL, then continue through deploy status checks and cloud verification. Do not deploy pure documentation, tech plan, PRD, or comment-only changes unless those files are part of the served product surface or the user explicitly asks for deployment.
