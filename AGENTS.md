...
- A Delivery Coordinator that dispatches another role/session must establish an active watch path. Use an available heartbeat/monitor, keep checking the child thread, or record `Monitoring not active` with the exact resume action; passive "I will wait" is not enough.
