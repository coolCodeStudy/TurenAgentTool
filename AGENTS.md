...
- Daily cloud releases should use the pull-based `/ops/deploy quick` path through the independent ECS Ops API. Use full deploy only for `Dockerfile`, `requirements.txt`, `docker-compose.prod.yml`, image/build/dependency, or production service-structure changes. Documentation-only and tests-only changes do not justify full deploy by themselves.
...
- Manual `full` deploy requires an explicit Deploy Intent reason tied to Docker, dependency, Compose, image-layer, or service-structure risk. "Just in case" is not a sufficient reason.
