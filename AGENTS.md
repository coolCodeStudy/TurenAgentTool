...
- Track operating-model infrastructure plans in `docs/project-management/Agent-Operating-Model-Roadmap.md`; track active execution in `docs/project-management/Delivery-Queue.md`. Do not put internal agent-system improvements into `Feature-Registry.md` unless they create a user-facing product feature.
...
- A Delivery Coordinator watch path must be a watch contract, not prose. It must name the watched child thread/session or queue item, the wake event or check cadence, the expected return artifact, and what the coordinator will do when the event happens.
- When a coordinator creates a visible Codex thread for Product, Development, Acceptance, or another long-running role, it must also create or reuse a runtime watcher such as a heartbeat/monitor in the owning coordinator thread, unless the handoff explicitly records `Monitoring not active` and the smallest resume action. Writing a watch contract in docs is not enough if no process will wake the coordinator.
