...
- Use prompt templates under `docs/project-management/prompt-templates/` for repeatable Feature Coordinator, Release Integration, Acceptance Testing, Deploy Conflict Resolution, and Frontend Review dispatches.
...
- Governance-only changes may be pushed to `main` after verification when they match the GitHub Actions `no_deploy` classification: `AGENTS.md`, `docs/**`, markdown-only changes, local audit/eval scripts such as `scripts/audit_agent_flow_health.py`, `scripts/audit_delivery_state.py`, `scripts/audit_prd_status.py`, `scripts/agent_preflight.py`, `scripts/evaluate_agent_flow_cases.py`, and workflow-file changes under `.github/workflows/**`. Do not hold these changes on feature branches only because of deploy fear; the workflow should skip production deployment for them.
