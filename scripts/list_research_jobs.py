...
def main() -> None:
    ...
    parser.add_argument("--full", action="store_true", help="Include full artifacts JSON and markdown fields.")
    args = parser.parse_args()

    run_schema()
    status = None if args.status == "all" else args.status
    rows = list_research_jobs(status=status, limit=args.limit, verbose=args.full)
