...
| Frontend experience system | [`PRD-Frontend-Experience-System.md`](../product/PRD-Frontend-Experience-System.md) | ready | missing | missing | not_started | doc_reference | not_required | Product PRD chooses the recommended two-surface first slice: shared server-rendered app shell, primary navigation, and design-token contract for `/weekly-review` and `/command`. Daily Market Brief remains planning-only with no route/package/renderer and must not get a fake nav affordance. Technical plan, implementation, deployment, independent acceptance testing, and user acceptance are still missing. | Feature Coordinator should apply Return Gate to Product PRD row `DQ-2026-07-05-002`, then dispatch Development Agent to create the technical plan. Frontend Experience Reviewer is required before implementation handoff; Release Reviewer is required before deploy if both `/weekly-review` and `/command` change in one release; Acceptance Reviewer is required after the deployed user-facing slice. |
