...
| Daily market brief history access | [`PRD-Daily-Market-Brief.md`](../product/PRD-Daily-Market-Brief.md) | ready | [`daily-market-brief.md`](../techplans/daily-market-brief.md) | implemented | needs_deploy | local_verified | pending | Adds public single-date historical async generation, persistent job progress, saved partial historical reports, authenticated batch backfill/status/cancel, a dedicated one-item global worker, sanitized command failures, and localhost-only MCP host binding by default. See the Historical Access and Historical Jobs sections. Local focused and tests-directory verification passed on the feature branch; independent reviewer findings about public batch leakage, failed-result sanitization, latest-session batch inclusion, and provider-thread lifecycle were fixed. | Merge and deploy branch `codex/daily-market-brief-history`, then run cloud acceptance for `/daily-market-brief` historical date `2026-07-09`, public job visibility boundaries, authenticated batch controls, worker health, and no raw internal error leakage. |
