...
| Daily market brief | [`PRD-Daily-Market-Brief.md`](../product/PRD-Daily-Market-Brief.md) | ready | [`daily-market-brief.md`](../techplans/daily-market-brief.md) | partially_implemented | local_verified | test_passed | pending | P0 command surface, `daily_market_brief` persistence helpers, scheduler-ready module entrypoint, Simplified Chinese Markdown rendering, source-status metadata, explicit capital-flow degraded states, weekend no-session handling, and command retrieval for latest/specified market/date are implemented. Acceptance retest `AT-2026-06-30-002` previously failed/major because fixture commands omitted required core indexes, raw Yahoo/SSL provider text leaked in `数据状态`, and legacy `report_key` handling collided across CN/HK same-date rows. Development fix on branch `codex/daily-market-brief-acceptance-fix` now provides deterministic fixture index bars through `scripts/ikg.py`, sanitizes provider/degraded copy into product language, and makes daily-brief `report_key` market-aware. Verification passed: `.venv/bin/python -m unittest tests.test_daily_market_brief`, `.venv/bin/python -m py_compile investment_knowledge_mcp/daily_market_brief.py investment_knowledge_mcp/command_router.py investment_knowledge_mcp/repository.py investment_knowledge_mcp/market_data_provider.py`, `git diff --check`, and DB-backed CLI smokes on existing local PostgreSQL `POSTGRES_PORT=55433` for CN/HK/US fixture generation, CN latest/specific retrieval, HK/US specific retrieval, CN same-market rerun idempotency (`review_reports #130` reused), CN weekend/no-session generation (`#133`), US live/degraded generation (`#135`), and report-key metadata (`daily_market_brief:CN/HK/US:<date>:<date>`). Known live-data gap remains: current configured providers do not supply full-market sector/industry rankings, liquid common-stock gainer universes, or explicit capital-flow metrics; live generation degrades visibly without raw internals. | Feature Coordinator should integrate the Development fix, keep user acceptance pending, and dispatch Acceptance Testing to rerun `AT-2026-06-30-002`; do not ask the user for acceptance until the independent retest passes. |
