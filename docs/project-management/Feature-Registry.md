...
| Daily market brief | [`PRD-Daily-Market-Brief.md`](../product/PRD-Daily-Market-Brief.md) | ready | [`daily-market-brief.md`](../techplans/daily-market-brief.md) | partially_implemented | local_verified | test_passed | pending | P0 command surface, `daily_market_brief` persistence helpers, market/date idempotency, scheduler-ready module entrypoint, CN/HK/US index generation, Simplified Chinese Markdown rendering, source-status metadata, fixture-backed verification, explicit capital-flow degraded states, weekend no-session handling, and command retrieval for latest/specified market/date are implemented on branch `codex/daily-market-brief-dev`. Verification passed: `.venv/bin/python -m unittest tests.test_daily_market_brief`, `.venv/bin/python -m py_compile investment_knowledge_mcp/daily_market_brief.py investment_knowledge_mcp/command_router.py investment_knowledge_mcp/repository.py investment_knowledge_mcp/market_data_provider.py`, `git diff --check`, and `python3 scripts/audit_delivery_state.py --feature "Daily market brief"`. Known live-data gap: current configured providers do not supply full-market sector/industry rankings, liquid common-stock gainer universes, or explicit capital-flow metrics; live generation records degraded states rather than inventing those rows, while fixtures verify rendering and command shape. | Feature Coordinator should apply the Development return gate, integrate or reject branch `codex/daily-market-brief-dev`, then dispatch Acceptance Testing for `AT-2026-06-30-002` using command examples `生成每日市场简报 CN 2026-06-30 fixture`, `每日市场简报 CN 2026-06-30`, and `最新每日市场简报 CN`. A future provider task is needed for live full-market sectors/gainers/flow beyond degraded states. |
