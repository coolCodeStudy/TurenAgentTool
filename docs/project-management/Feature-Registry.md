...
| Command Workbench | [`PRD-command-workbench.md`](../product/PRD-command-workbench.md) | ready | [`command-workbench.md`](../techplans/command-workbench.md) | implemented | deployed | deploy_verified | pending | Bounded V1 is implemented for registry-backed parsing, preview, candidate selection, confirmation guards, `/command` UI, workbench APIs, and Level 1 decision-card execution. Cloud acceptance is served at `:8010/command` because `command-api` runs internally but `:8001` is not publicly reachable. Decision-ticket/history actions recover explicitly because only Level 1 decision cards exist today. Full smoke test was blocked by local PostgreSQL `localhost:55432` being unavailable. 2026-06-29 Development/Ops restored durable public `weekly-review-web` token wiring on latest `main` lineage via branch `origin/codex/command-workbench-runtime-token-main` commit `6f0a207` and full deploy run `28384150041`; cloud verification now confirms `/command` 200, action catalog 200 with 16 actions, missing/invalid auth recoverable 401 rather than configuration 503, valid-token parse preview for `决策 英特尔`, valid-token read-only execute for `系统状态`, and confirmation guard 409 for `本周复盘`. Root cause was a later full deploy from `main` that recreated `weekly-review-web` from a ref missing `COMMAND_API_TOKEN` compose wiring. User acceptance is still pending; do not mark accepted until the user verifies the behavior after independent acceptance passes. | Return to Delivery Coordinator for Acceptance Testing redispatch of `AT-2026-06-25-002` from `needs_retest`. Keep cloud `:8010/command` available for user acceptance after acceptance testing passes; revisit separate decision-ticket/history scope if product still requires it. |
