...
| Daily market brief | [`PRD-Daily-Market-Brief.md`](../product/PRD-Daily-Market-Brief.md) | ready | [`daily-market-brief.md`](../techplans/daily-market-brief.md) | partially_implemented | local_verified | test_passed | pending | P0 command surface, `daily_market_brief` persistence helpers, market/date idempotency, scheduler-ready module entrypoint, CN/HK/US index generation, Simplified Chinese Markdown rendering, source-status metadata, fixture-backed verification, explicit capital-flow degraded states, weekend no-session handling, and command retrieval for latest/specified market/date are implemented on integrated coordinator commit `62713eb`, but independent Acceptance retest `AT-2026-06-30-002` failed/major on 2026-06-30 SGT. Passing evidence still includes `.venv/bin/python -m unittest tests.test_daily_market_brief`, `.venv/bin/python -m py_compile investment_knowledge_mcp/daily_market_brief.py investment_knowledge_mcp/command_router.py investment_knowledge_mcp/repository.py investment_knowledge_mcp/market_data_provider.py`, and same-market CN command rerun reusing `review_reports #130`. Failed acceptance evidence: real `scripts/ikg.py ... fixture` output omitted required core indexes for CN/HK, exposed raw Yahoo/SSL provider exception text in user-facing `数据状态`, and multi-market persistence/schema handling collided on `report_key=daily_market_brief:2026-06-30:2026-06-30` after CN and HK briefs existed for the same date, blocking US and weekend/no-session CLI checks. Known live-data gap remains: current configured providers do not supply full-market sector/industry rankings, liquid common-stock gainer universes, or explicit capital-flow metrics; live generation may degrade, but it must not hide gaps, invent rows, expose raw internals, or break cross-market storage. | Feature Coordinator should apply the Acceptance Return Gate, reject readiness for user acceptance, and dispatch Development to fix command-surface fixture index generation, product-language degraded-state sanitization, and market-aware `review_reports` idempotency/schema handling before moving `AT-2026-06-30-002` back to `needs_retest`. |
