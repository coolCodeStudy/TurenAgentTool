...
| Frontend experience system | [`PRD-Frontend-Experience-System.md`](../product/PRD-Frontend-Experience-System.md) | ready | missing | missing | not_started | doc_reference | not_required | Product PRD chooses the recommended two-surface first slice: shared server-rendered app shell, primary navigation, and design-token contract for `/weekly-review` and `/command`. Daily Market Brief remains planning-only with no route/package/renderer and must not get a fake nav affordance. Technical plan, implementation, deployment, independent acceptance testing, and user acceptance are still missing. | Product PRD Return Gate is complete in `DQ-2026-07-05-002`; Development Agent is dispatched in `DQ-2026-07-05-003` to create `docs/techplans/frontend-experience-system.md`. Frontend Experience Reviewer is required before implementation handoff; Release Reviewer is required before deploy if both `/weekly-review` and `/command` change in one release; Acceptance Reviewer is required after the deployed user-facing slice. |
