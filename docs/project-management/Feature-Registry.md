...
| Frontend experience system | Product context pending | missing | missing | missing | not_started | none | not_required | Product surfaces are growing as separate page islands, especially Command Workbench, Weekly Review, and Daily Market Brief. Current Web UI is mostly embedded inline in Python renderers, so navigation, layout, component reuse, responsive behavior, and code ownership need an explicit experience architecture before more pages are added. This inventory/planning phase does not need independent acceptance testing until it becomes a user-facing UI implementation. | Dispatch a Feature Coordinator / Product Experience owner to inventory existing pages, propose target information architecture, define shared UI/navigation rules, and recommend whether to keep inline renderers, introduce templates/static assets, or move toward a dedicated frontend layer. |
...
| Stock valuation research | [`PRD-Stock-Valuation-Research.md`](../product/PRD-Stock-Valuation-Research.md) | ready | [`stock-valuation-research.md`](../techplans/stock-valuation-research.md) | implemented | needs_deploy | code_verified | needs_reacceptance | P0 single-stock command flow and P0.1 provider-backed packet are implemented, cloud-deployed, independently acceptance-tested, and user accepted on 2026-07-04 SGT; do not reopen P0/P0.1. P0.2 product addendum from Product commit `3dbdf3f` is implemented and deployed through superseding combined releases preserving Stock valuation behavior and Kline live `yahoo_chart` provider behavior. Independent Acceptance retest `AT-2026-07-04-002` initially failed major because the fresh `valuation US.INTC` card exposed raw provider error text in user-facing degraded-state copy. Global PM returned combined release `origin/codex/release-kline-stock-valuation-copy-fix` at `3e78c5a0af18b57561074a2afb0cb6fa80c9382b`, and Coordinator quick-deployed it through Ops event `#50`. Independent Acceptance retest on 2026-07-05 SGT passed: `/command` 200, 22-action catalog with required Kline/valuation IDs, tokened `valuation US.INTC` event `#429` without raw HTTP/provider diagnostics, tokened `valuation artifact evidence US.INTC` event `#431` with bounded evidence and no token/auth/header/trace/config/raw-provider-error/local-path/file-read leakage, path-like artifact probe safe 409, Kline `K线调查 US.NVDA 5年 前复权` event `#433` with `Provider: yahoo_chart`, and no token leakage in scoped artifacts under `/private/tmp/stock-valuation-copy-fix-3e78c5a-acceptance/`. User explicitly accepted INTC P0.2 on 2026-07-05 SGT. P0.3 Development local implementation now covers fixture-scoped non-US provider coverage for SK hynix `KR.000660` / `000660.KS` and Kingboard Laminates `HK.01888` / `1888.HK`: target mapping, HKD/KRW display, source-attempt taxonomy, vendor-labeled fallback fundamentals, market-implied bridge/recovery behavior, and sanitized card/evidence output. Combined P0.3 release `245a3c4` was deployed through Ops event `#51`, but coordinator cloud smoke blocked acceptance because `valuation 建滔积层板 HK` routed to stock-profile bootstrap and HK output lacked explicit `HKD`. Development fixed the updated combined branch so simplified/traditional Chinese and `Kingboard Laminates HK` aliases route to `stock_valuation` for `HK.01888` even without a Workbench stock-profile row, and HK fixture rows include explicit `HKD` labels while preserving `HK$` formatting. P0/P0.1/P0.2 remain accepted; P0.3 still needs redeploy, coordinator cloud smoke, and independent cloud-IP retest. Peer multiples, analyst estimates, normalized valuation tables, async refresh, user-confirmed valuation-case workflow, and broad global market data coverage remain later follow-up scope. | P0/P0.1/P0.2 are accepted. Stock valuation Feature Coordinator should apply the Return Gate to the returned Development commit on `codex/release-kline-stock-valuation-p03`, then `self_deploy` the updated combined branch through the shared Ops API path if accepted. After deploy, verify `/command`, required Workbench actions, P0.3 KR/HK valuation commands including `valuation 建滔积层板 HK`, `valuation 建滔積層板 HK`, and `valuation Kingboard Laminates HK`, existing `valuation US.INTC`, Kline `kline_investigation` with live provider lineage, and token secrecy, then move `AT-2026-07-05-003` to `needs_retest` only if coordinator smoke passes. Do not ask the user for P0.3 acceptance until the retest passes. |
| Kline Agent | [`PRD-Kline-Agent.md`](../product/PRD-Kline-Agent.md) | ready | [`kline-agent-v1.md`](../techplans/kline-agent-v1.md) | implemented | deployed | needs_review | pending | V1 command/CLI and Command Workbench routing exist for read-only daily/weekly/monthly Kline investigation, deterministic rules, fixture statistics, data-quality warnings, and degraded-provider reporting. Local FutuD/OpenD remains forbidden; approved remote/cloud Futu is allowed when configured. Development commit `0fd67b2` adds a no-credential `yahoo_chart` daily OHLCV provider for common U.S. stocks plus normal Workbench execution so `/command` no longer forces degraded Kline runs. The cloud release repeatedly lost this commit during cross-feature deploys: Kline passed in event #313 after Ops #42, failed in event #333 after cross-feature deploy #43 at `f19cb9494594`, passed again in event #337 after reconciled Ops #44 at `5a941d3`, and then failed again in the user's fresh browser event #402 after cloud `system_overview` reported latest deploy #48 quick commit `880b741393ad`. Global PM combined releases now merge Kline live-provider lineage with Stock valuation release lines; latest Stock P0.3 combined branch is `origin/codex/release-kline-stock-valuation-p03`. Remaining gaps: deploy/browser retest when a Kline acceptance item is active, AkShare/secondary fallback, durable bar/run cache schema, portfolio/watchlist scans, decision-card/weekly-review integration, and user acceptance. | Preserve `kline_investigation` and live `yahoo_chart` provider behavior in every cross-feature release. For this Stock valuation P0.3 release, verify Kline action/catalog and a focused Kline command before deploy handoff closure. |
...
- Frontend experience system: product context and target information architecture are missing.
