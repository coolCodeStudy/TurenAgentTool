...
| Daily market review | [`PRD-Daily-Market-Review.md`](../product/PRD-Daily-Market-Review.md) | ready | [`daily-market-review.md`](../techplans/daily-market-review.md) | partially_implemented | local_verified | test_passed | Command/report/storage/scoring path is implemented and smoke-tested with fake providers; live probe currently shows Futu and AkShare dependencies unavailable in this environment, so real market coverage is not yet usable. | Enable or install approved live providers, rerun `scripts/probe_daily_market_providers.py`, then verify a real-data report. |
