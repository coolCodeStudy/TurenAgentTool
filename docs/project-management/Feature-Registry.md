...
| Frontend experience system | [`PRD-Frontend-Experience-System.md`](../product/PRD-Frontend-Experience-System.md) | ready | [`frontend-experience-system.md`](../techplans/frontend-experience-system.md) | implemented | deployed | deploy_verified | pending | Development branch `codex/frontend-experience-shell` implements the first two-surface slice with shared server-rendered shell/navigation/design tokens in `investment_knowledge_mcp/frontend_shell.py`, applied to `/`, `/weekly-review`, and `/command` through the existing route owners. Daily Market Brief remains omitted because it has no route/package/renderer. Revision branch `codex/frontend-shell-parser-fix` adds rendered JavaScript parser assertions, verifies Weekly Review rendered CSS has no f-string doubled braces, and fixes Command Workbench rendered result-card regex parsing. Local evidence includes py_compile, focused renderer shell/regression/parser assertions, desktop/mobile browser screenshots for `/weekly-review` and `/command` from the original implementation return, and delivery audits; full `scripts/smoke_test.py` remains partially limited by the local Futu gateway environment. Release Reviewer passed exact reviewed ref `4e0abc3752933b53e865ecaaf8ec4d0190da7b6b`. Coordinator deployed that ref through shared Ops API quick deploy event `#53`, which succeeded with resolved commit match and health OK. Coordinator in-app browser verification reached public `/`, `/weekly-review`, and `/command`, confirming shared primary nav, correct active nav, no Daily Market Brief affordance, and preserved Weekly Review/Command Workbench API/token strings. Independent acceptance test is dispatched but has not returned, and user acceptance is not accepted. | Acceptance Reviewer is dispatched in `DQ-2026-07-05-010` for `AT-2026-07-05-001` against deployed `/`, `/weekly-review`, and `/command`. The Feature Coordinator owns the watch heartbeat `frontend-experience-acceptance-return-watch`; on Acceptance Reviewer return, apply Return Gate and either ask Owner for user acceptance if passed or route fixes if failed/blocked. |
