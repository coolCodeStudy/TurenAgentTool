...
| Stock valuation research | [`PRD-Stock-Valuation-Research.md`](../product/PRD-Stock-Valuation-Research.md) | ready | [`stock-valuation-research.md`](../techplans/stock-valuation-research.md) | implemented | deployed | deploy_verified | needs_reacceptance | P0 single-stock command flow and P0.1 provider-backed packet are implemented, cloud-deployed, independently acceptance-tested, and user accepted on 2026-07-04 SGT; do not reopen P0/P0.1. P0.2 product addendum from Product commit `3dbdf3f` is implemented and deployed through superseding combined releases preserving Stock valuation behavior and Kline live `yahoo_chart` provider behavior. Independent Acceptance retest `AT-2026-07-04-002` on 2026-07-04 SGT found the artifact evidence command itself passed raw/display/meaningfulness/provider/bridge/frame-fit checks, but failed major because the fresh `valuation US.INTC` card exposed raw provider error text `HTTP Error 401: Unauthorized` in user-facing degraded-state copy. Global PM returned combined release `origin/codex/release-kline-stock-valuation-copy-fix` at `3e78c5a0af18b57561074a2afb0cb6fa80c9382b`, and Coordinator quick-deployed it through Ops event `#50`. Coordinator cloud smoke verified `/command` 200, 22-action catalog with required Kline/valuation IDs, tokened `valuation US.INTC` event `#423` without raw HTTP/provider diagnostics, tokened `valuation artifact evidence US.INTC` event `#425`, Kline `K线调查 US.NVDA 5年 前复权` event `#427` with `Provider: yahoo_chart`, and no token leakage in scoped artifacts under `/tmp/stock-valuation-copy-fix-3e78c5a-cloud/`. Peer multiples, analyst estimates, normalized valuation tables, async refresh, and user-confirmed valuation-case workflow remain later follow-up scope. | Next owner is Acceptance Testing via `DQ-2026-07-04-025`: run the narrow independent cloud-IP retest for P0.2 copy-fix, artifact evidence, token secrecy, and Kline preservation. Do not ask the user for P0.2 acceptance until the retest passes. |
