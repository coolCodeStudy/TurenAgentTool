...
| Kline Agent | [`PRD-Kline-Agent.md`](../product/PRD-Kline-Agent.md) | ready | [`kline-agent-v1.md`](../techplans/kline-agent-v1.md) | implemented | deployed | deploy_verified | pending | V1 command/CLI and Command Workbench routing exist for read-only daily/weekly/monthly Kline investigation, deterministic rules, fixture statistics, data-quality warnings, and degraded-provider reporting. Local FutuD/OpenD remains forbidden; approved remote/cloud Futu is allowed when configured, and Development commit `0fd67b2` adds a no-credential `yahoo_chart` daily OHLCV provider for common U.S. stocks plus normal Workbench execution so `/command` no longer forces degraded Kline runs. Branch head `db7c1e9` was quick-deployed through Ops event #42 and event #313 returned `Provider: yahoo_chart`; later cross-feature deploy #43 at `f19cb9494594` replaced that release with a ref missing `0fd67b2`, causing coordinator acceptance event #333 to degrade to `KLINE_PROVIDER=disabled`. Release/Deploy owner reconciled the latest stock-valuation release with Kline live-provider code in branch `codex/kline-live-provider-reconcile` at `5a941d3`, quick-deployed it through Ops event #44, and verified event #337 on `http://47.84.190.191:8010/command` returned `Provider: yahoo_chart`, `Provider symbol: NVDA`, market/currency/timezone metadata, requested and actual date ranges, adjustment/fetch time, raw and normalized bar counts of 1257, daily/weekly/monthly facts and statistics, visible Yahoo adjustment warning, and no buy/sell/hold or `买入`/`卖出`/`持有` recommendation. Coordinator final browser retest also passed with evidence `/tmp/kline-live-coordinator-final-retest-20260704.json` and `/tmp/kline-live-coordinator-final-retest-20260704.png`; `AT-2026-07-01-001` is passed/minor because independent Acceptance agents were inactive, but cloud product behavior passed. Remaining gaps: AkShare/secondary fallback, durable bar/run cache schema, portfolio/watchlist scans, decision-card/weekly-review integration, and user acceptance. | Ready for user review on `http://47.84.190.191:8010/command`; ask the user to review exact command `K线调查 US.NVDA 5年 前复权`. User acceptance remains pending until explicitly accepted by the user. |
| Stock valuation research | [`PRD-Stock-Valuation-Research.md`](../product/PRD-Stock-Valuation-Research.md) | ready | [`stock-valuation-research.md`](../techplans/stock-valuation-research.md) | implemented | needs_deploy | code_verified | needs_reacceptance | P0 single-stock command flow is already cloud-deployed through combined release `658bbfc` / Ops deploy event `#41`, and independent cloud-IP acceptance previously passed `AT-2026-07-01-001`. User acceptance feedback on 2026-07-04 showed the `US.INTC` valuation card was too thin because the implemented P0 only used existing local stock context and did not fetch fresh provider data. P0.1 now adds a US provider-backed valuation packet in combined release branch `origin/codex/release-access-kline-stock-valuation-provider`: SEC EDGAR company facts for official financial metrics and Yahoo quote data for low-cost market snapshots, merged into deterministic calculations and saved artifacts with source labels and provider-gap degraded behavior. This branch preserves Command Workbench access setup and Kline behavior from `658bbfc`; P0.1 is code-verified but not yet cloud-deployed. Peer multiples, analyst estimates, normalized valuation tables, async refresh, and user-confirmed valuation-case workflow remain follow-up scope. | Next owner is the Stock valuation Feature Coordinator: deploy `origin/codex/release-access-kline-stock-valuation-provider` through the shared Ops API path, verify `/command`, action catalog, private-token `valuation US.INTC`, provider-backed calculations/source labels, keep `AT-2026-07-01-001` at `needs_retest`, and redispatch independent cloud-IP Acceptance Testing. Do not ask the user for final acceptance until retest passes. |
