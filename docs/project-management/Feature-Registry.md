...
| Command Workbench | [`PRD-command-workbench.md`](../product/PRD-command-workbench.md) | ready | [`command-workbench.md`](../techplans/command-workbench.md) | implemented | deployed | deploy_verified | pending | Bounded V1 is implemented for registry-backed parsing, preview, candidate selection, confirmation guards, `/command` UI, workbench APIs, and Level 1 decision-card execution. Cloud acceptance is served at `:8010/command` because `command-api` runs internally but `:8001` is not publicly reachable. Decision-ticket/history actions recover explicitly because only Level 1 decision cards exist today. Full smoke test was blocked by local PostgreSQL `localhost:55432` being unavailable. 2026-06-29 product decision: P0 may use one fixed private owner/test token configured outside the repo, stored only in browser localStorage by the `/command` page; multi-user login, OAuth, short-lived token issuance, rotation UI, and separate read/write tokens are P1 follow-ups. 2026-06-29 Engineering token UX fix is integrated and deployed from coordinator commit `95f35c5` via GitHub Actions run `28381075549`; cloud checks confirm `/command` 200 with browser-local token copy and missing/invalid parse plus missing execute return 401 product-language recovery copy. 2026-06-29 Coordinator generated a fixed owner/test token outside git, configured it as the GitHub Actions `COMMAND_API_TOKEN` secret, full-deployed coordinator commit `0b0be59` via run `28382760062`, and initially verified valid-token parse/preview works against cloud without exposing the token value. 2026-06-29 Acceptance Testing retest then found the runtime token missing; Development/Ops traced the cause to a later full deploy from `main` recreating `weekly-review-web` from a ref without `COMMAND_API_TOKEN` wiring. Development/Ops restored token wiring with full deploy run `28384150041` on runtime code commit `6f0a207` and quick deploy run `28384624938` on final commit `55bd671`. 2026-06-29 independent acceptance retest passed for `AT-2026-06-25-002` with sanitized API evidence `/tmp/command-workbench-acceptance-retest-20260629.json` and browser evidence `/tmp/command-workbench-browser-retest-20260629.json`; explicit buy/sell text remains unsupported and non-executable, while the edge phrase `乱买英特尔` may map to a read-only decision preview. User acceptance is still pending; do not mark accepted until the user verifies the behavior after acceptance passes. | Return the passed acceptance result to the Delivery Coordinator. Keep cloud `:8010/command` available and ask the user for acceptance only through the Coordinator/user-acceptance gate. |
