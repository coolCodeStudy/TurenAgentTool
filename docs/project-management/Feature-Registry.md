...
| Daily market brief | [`PRD-Daily-Market-Brief.md`](../product/PRD-Daily-Market-Brief.md) | ready | missing | missing | not_started | doc_reference | pending | Product defaults are now recorded for P0: structured `review_reports` persistence with `daily_market_brief`, command retrieval for latest/specified market/date, exact CN/HK/US index lists, configured no-paid-dependency providers with native provider taxonomies, explicit capital-flow semantics and degraded copy, liquid common-stock gainer universe, previous-day/5-day/20-day volume or turnover baselines, Simplified Chinese output, market-local plus Asia/Singapore timestamps, and post-close schedules. Acceptance row `AT-2026-06-30-002` is blocked until Engineering creates an implemented command surface, scheduler, persisted report, and verification evidence. | Engineering should create `docs/techplans/daily-market-brief.md`, then implement P0 storage/idempotency, scheduler, provider/source-status handling, command retrieval, and verification fixtures. After implementation, move `AT-2026-06-30-002` to `needs_retest` and dispatch Acceptance Testing. |
