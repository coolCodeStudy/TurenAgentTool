...
| Weekly review generator | [`PRD-每周复盘.md`](../product/PRD-每周复盘.md) | ready | [`weekly-review.md`](../techplans/weekly-review.md) | partially_implemented | deployed | deploy_verified | rejected | Core weekly review generation, persistence, command path, and web surface exist. 2026-06-28 source-completeness branch `origin/codex/weekly-review-source-completeness` at `ce36764` was integrated into `main` through `8147cb8` and full-deployed by GitHub Actions run `28326192300`. Cloud HTML now renders the new index slot and no longer contains the old fixed provider-not-connected copy. Cloud force-refresh for week `2026-06-22` generated the new story keys and local knowledge evidence, but source completeness is still blocked: `source_status.indexes.status=provider_unavailable`, `source_status.events.status=source_blocked`, `index_summary=[]`, and `event_summary=[]`. | Do not route `AT-2026-06-28-001` to Acceptance Testing yet. Engineering/Ops must make cloud index provider and external event/company evidence available, or Product must explicitly lower the source-completeness acceptance bar; after a successful cloud force-refresh with index/event evidence, move `AT-2026-06-28-001` to `needs_retest`. |
