...
| Stock valuation research | [`PRD-Stock-Valuation-Research.md`](../product/PRD-Stock-Valuation-Research.md) | ready | [`stock-valuation-research.md`](../techplans/stock-valuation-research.md) | implemented | local_verified | test_passed | pending | P0 single-stock command flow is implemented and local-verified after Coordinator integration commit `4d220f9`: `valuation` / `value` / `估值` creates a valuation card and saved JSON artifact; `查看估值` reads the latest artifact; `估值方法` lists the five internal core frames. The artifact-backed implementation preserves input fields, source IDs when available, timestamps, deterministic calculations, all internal frame scores, selected user-facing frames, assumptions, source coverage, degraded state, and safety flags. Prior independent acceptance used local fixture command-router/Workbench checks because local Postgres `127.0.0.1:55432` refused connections, but user correction on 2026-07-03 clarified that user acceptance must be limited to the cloud system IP, not local browser or fixture flow. Provider-backed official filings, fresh market snapshots, peer multiples, dedicated valuation tables, async refresh, and user-confirmed valuation-case workflow remain follow-up scope. | Do not ask the user for acceptance yet. Deploy decision is `self_deploy`: the Feature Coordinator should push the coordinator branch, quick-deploy merge commit `f62f2e9` through the shared Ops API path to make this P0 available on the cloud Command Workbench at `http://47.84.190.191:8010/command`, verify deploy health, then dispatch Acceptance Testing for a cloud-IP retest of `AT-2026-07-01-001`. |
