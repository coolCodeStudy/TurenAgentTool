...
| Kline Agent | [`PRD-Kline-Agent.md`](../product/PRD-Kline-Agent.md) | ready | [`kline-agent-v1.md`](../techplans/kline-agent-v1.md) | implemented | deployed | needs_review | pending | V1 command/CLI and Command Workbench routing exist for read-only daily/weekly/monthly Kline investigation, deterministic rules, fixture statistics, data-quality warnings, and degraded-provider reporting. Local FutuD/OpenD remains forbidden; approved remote/cloud Futu is allowed when configured, and Development commit `0fd67b2` adds a no-credential `yahoo_chart` daily OHLCV provider for common U.S. stocks plus normal Workbench execution so `/command` no longer forces degraded Kline runs. Branch head `db7c1e9` was quick-deployed through Ops event #42 to `http://47.84.190.191:8010/command` and deploy verification event #313 returned `Provider: yahoo_chart`, `Provider symbol: NVDA`, market/currency/timezone metadata, requested and actual date ranges, adjustment/fetch time, raw and normalized bar counts of 1257, daily/weekly/monthly facts and statistics, visible Yahoo adjustment warning, and no buy/sell/hold or `买入`/`卖出`/`持有` recommendation. However, the later coordinator browser acceptance check on the same cloud surface returned event #333 with `Provider: unavailable` and warning `KLINE_PROVIDER=disabled`, no bar count, and no daily/weekly/monthly statistics. Evidence: deploy verification `/tmp/kline-live-provider-deploy-verification-20260704.json` plus `/tmp/kline-live-provider-deploy-verification-20260704.png`; current failed acceptance `/tmp/kline-live-acceptance-coordinator-failed-20260704.json` plus `/tmp/kline-live-acceptance-coordinator-failed-20260704.png`. Remaining gaps: diagnose cloud runtime/config drift, rerun acceptance for `AT-2026-07-01-001`, AkShare/secondary fallback, durable bar/run cache schema, portfolio/watchlist scans, decision-card/weekly-review integration, and user acceptance. | Kline Feature Coordinator closed inactive acceptance dispatch `DQ-2026-07-04-007` as failed/major and dispatched Release/Deploy owner in `DQ-2026-07-04-008` to fix current cloud `KLINE_PROVIDER=disabled` behavior before retesting or asking for user acceptance. |
