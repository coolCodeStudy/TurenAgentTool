...
| Stock valuation research | [`PRD-Stock-Valuation-Research.md`](../product/PRD-Stock-Valuation-Research.md) | ready | [`stock-valuation-research.md`](../techplans/stock-valuation-research.md) | implemented | needs_deploy | code_verified | needs_reacceptance | P0 single-stock command flow and P0.1 provider-backed packet are implemented, cloud-deployed, independently acceptance-tested, and user accepted on 2026-07-04 SGT; do not reopen P0/P0.1. P0.2 visible Workbench behavior is implemented through combined release `origin/codex/release-access-kline-stock-valuation-p02` at `d13f32023c3cc0ba0d587affd699835ecc74b159`, preserving Command Workbench access setup and Kline Workbench behavior while adding readable currency/percent/multiple formatting, provider-gap taxonomy, negative/meaningless labels, market-implied bridge lines, and frame-fit ranking. Global PM release `origin/codex/release-kline-stock-valuation-artifact-evidence-live-kline` superseded the earlier artifact-evidence-only release by adding bounded `valuation artifact evidence US.INTC` / `valuation evidence US.INTC` / `估值证据 US.INTC` commands from commit `40017ea` while also preserving Kline live-provider commit `0fd67b2`, Kline `kline_investigation`, and Command Workbench access setup. Independent P0.2 retest then failed only because fresh `valuation US.INTC` card copy exposed raw provider diagnostics in degraded-state copy. Global PM release `origin/codex/release-kline-stock-valuation-copy-fix` adds provider-gap copy fix commit `31fbfb1` on top of the live Kline + Stock artifact-evidence line so raw HTTP/provider diagnostics stay out of user-facing cards and black-box evidence read-back. Peer multiples, analyst estimates, normalized valuation tables, async refresh, and user-confirmed valuation-case workflow remain later follow-up scope. | Deploy `origin/codex/release-kline-stock-valuation-copy-fix` through the shared Ops API path, verify `/command`, action catalog, private-token `valuation US.INTC` sanitized provider-gap copy, `valuation artifact evidence US.INTC`, raw/display/meaningfulness/provider/bridge/frame-fit evidence payload, Kline live provider still returns `yahoo_chart`, and token secrecy. Then move `AT-2026-07-04-002` to `needs_retest` and rerun only the copy-fix/artifact-evidence read-back plus token-secrecy scan. Do not ask the user for P0.2 final acceptance while the row is blocked or `needs_retest`. |
| Kline Agent | [`PRD-Kline-Agent.md`](../product/PRD-Kline-Agent.md) | ready | [`kline-agent-v1.md`](../techplans/kline-agent-v1.md) | implemented | deployed | needs_review | pending | V1 command/CLI and Command Workbench routing exist for read-only daily/weekly/monthly Kline investigation, deterministic rules, fixture statistics, data-quality warnings, and degraded-provider reporting. Local FutuD/OpenD remains forbidden; approved remote/cloud Futu is allowed when configured. Development commit `0fd67b2` adds a no-credential `yahoo_chart` daily OHLCV provider for common U.S. stocks plus normal Workbench execution so `/command` no longer forces degraded Kline runs. The cloud release repeatedly lost this commit during cross-feature deploys: Kline passed in event #313 after Ops #42, failed in event #333 after cross-feature deploy #43 at `f19cb9494594`, passed again in event #337 after reconciled Ops #44 at `5a941d3`, and then failed again in the user's fresh browser event #402 after cloud `system_overview` reported latest deploy #48 quick commit `880b741393ad`. Global PM combined release `origin/codex/release-kline-stock-valuation-artifact-evidence-live-kline` now merges Kline live-provider branch `origin/codex/kline-live-provider-reconcile` with the latest Stock valuation artifact-evidence release `origin/codex/release-access-kline-stock-valuation-artifact-evidence`. Remaining gaps: deploy/browser retest, AkShare/secondary fallback, durable bar/run cache schema, portfolio/watchlist scans, decision-card/weekly-review integration, and user acceptance. | Deploy `origin/codex/release-kline-stock-valuation-artifact-evidence-live-kline` through the shared Ops API path, verify `/command`, action catalog, Kline `K线调查 US.NVDA 5年 前复权` with `Provider: yahoo_chart` and real bars/statistics, Stock valuation artifact-evidence commands, and token secrecy. Then move `AT-2026-07-01-001` to `needs_retest` and rerun Kline cloud acceptance. Do not ask the user for Kline acceptance while the row is failed or needs retest. |
