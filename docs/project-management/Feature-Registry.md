...
| Stock valuation research | [`PRD-Stock-Valuation-Research.md`](../product/PRD-Stock-Valuation-Research.md) | ready | [`stock-valuation-research.md`](../techplans/stock-valuation-research.md) | implemented | blocked | deploy_conflict | pending | P0 single-stock command flow is implemented and local-verified after Coordinator integration commit `4d220f9`: `valuation` / `value` / `估值` creates a valuation card and saved JSON artifact; `查看估值` reads the latest artifact; `估值方法` lists the five internal core frames. The artifact-backed implementation preserves input fields, source IDs when available, timestamps, deterministic calculations, all internal frame scores, selected user-facing frames, assumptions, source coverage, degraded state, and safety flags. User correction on 2026-07-03 clarified that user acceptance must be limited to the cloud system IP, not local browser or fixture flow. Global PM resolved the first Kline/Stock valuation deploy-ref conflict with release branch `origin/codex/release-kline-stock-valuation` at `5e90864`; Coordinator quick-deployed it through Ops API event `#38`, restoring Kline and valuation actions. The owner-access setup branch later solved the private token blocker and cloud token-bearing parse now succeeds, but full deploy event `#40` at commit `2c7ca130c13d55b8c979562912844a7141a383a3` overwrote the public catalog again: `/command` returns 200 and the action catalog includes `kline_investigation`, but omits `stock_valuation`, `stock_valuation_latest`, and `valuation_methods`. Provider-backed official filings, fresh market snapshots, peer multiples, dedicated valuation tables, async refresh, and user-confirmed valuation-case workflow remain follow-up scope. | Do not ask the user for acceptance yet. `AT-2026-07-01-001` is blocked by a global deploy/ref conflict, not by token access. `DQ-2026-07-04-006` asks Global PM for a combined release ref that preserves Command Workbench access setup, Kline behavior, and Stock valuation P0; after deploy, move acceptance back to `needs_retest` and redispatch cloud-IP Acceptance Testing. |
