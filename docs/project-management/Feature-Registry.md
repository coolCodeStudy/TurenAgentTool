...
| Deploy mode optimization | [`Delivery-Coordinator-Protocol.md`](../product/Delivery-Coordinator-Protocol.md) | ready | [`deploy-mode-optimization.md`](../techplans/deploy-mode-optimization.md) | implemented | local_verified | test_passed | pending | P0 adds a shared deploy-mode classifier, treats tests-only/docs-only changes as quick-compatible instead of full, keeps Docker/dependency/production Compose changes on full, and requires an explicit reason for manual full workflow dispatch. `no_deploy` is intentionally deferred because it changes production freshness semantics. | Global Project Manager should review branch `codex/deploy-mode-optimization`, integrate if acceptable, then allow the normal serialized GitHub Actions deploy path to run on `main` without separately dispatching manual full. |
