...
| Daily market brief | [`PRD-Daily-Market-Brief.md`](../product/PRD-Daily-Market-Brief.md) | ready | [`daily-market-brief.md`](../techplans/daily-market-brief.md) | partially_implemented | deployed | deploy_verified | pending | P0 command and Web surfaces exist for CN/HK/US daily close briefs, market/date idempotent storage, scheduler-ready generation entrypoints, core-index rows, source-status metadata, explicit degraded states, weekend/no-session handling, latest/specified retrieval, and Web review at `/daily-market-brief`. Command-surface acceptance `AT-2026-06-30-002` passed after fixing fixture index bars, product-language degraded status, and market-aware report keys. Web acceptance `AT-2026-07-01-002` passed locally, and cloud deploy run `28670512010` made `/daily-market-brief` available. User later reported the live page still lacked useful market data; coordinator added AKShare/Eastmoney provider wiring and fallback behavior, then stricter write auth plus Eastmoney/Sina HTTP fallbacks. Latest deployed coordinator commit is `9f59cd7` from full deploy run `28673608989`; cloud `/health` reports that release, `/daily-market-brief` returns 200, and unauthenticated generation returns 401. Local provider verification shows the Sina A-share fallback returns five liquid A-share gainers. Remaining live-data acceptance is blocked because the final cloud authenticated generation requires a valid `WEEKLY_REVIEW_WEB_TOKEN` or `COMMAND_API_TOKEN`; prior pre-fallback cloud diagnostics showed AKShare/Eastmoney connectivity failures, so the post-fallback cloud data counts must still be checked. HK/US sector and capital-flow rows remain product-language degraded until comparable providers are added. | Next owner is Daily Market Brief coordinator once token access is available: run authenticated cloud generation for CN/HK/US, inspect sectors/gainers/capital-flow counts and rendered degraded states, update `AT-2026-07-10-001`, then ask for user acceptance only if the live page is useful enough. Do not mark user acceptance accepted. |
