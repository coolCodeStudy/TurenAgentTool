...
| Frontend experience system | [`PRD-Frontend-Experience-System.md`](../product/PRD-Frontend-Experience-System.md) | ready | [`frontend-experience-system.md`](../techplans/frontend-experience-system.md) | implemented | local_verified | test_passed | pending | Development branch `codex/frontend-experience-shell` implements the first two-surface slice with shared server-rendered shell/navigation/design tokens in `investment_knowledge_mcp/frontend_shell.py`, applied to `/`, `/weekly-review`, and `/command` through the existing route owners. Daily Market Brief remains omitted because it has no route/package/renderer. Revision branch `codex/frontend-shell-parser-fix` adds rendered JavaScript parser assertions, verifies Weekly Review rendered CSS has no f-string doubled braces, and fixes Command Workbench rendered result-card regex parsing. Local evidence includes py_compile, focused renderer shell/regression/parser assertions, desktop/mobile browser screenshots for `/weekly-review` and `/command` from the original implementation return, and delivery audits; full `scripts/smoke_test.py` remains partially limited by the local Futu gateway environment. No deployment or independent acceptance test has run, and user acceptance is not accepted. | Next owner is Release Reviewer before deploy because both `/weekly-review` and `/command` change in one release. After Release Reviewer passes, a named deploy owner should deploy the pushed ref through the shared deploy path, then route Acceptance Reviewer for deployed `/weekly-review`, `/`, and `/command`; user acceptance remains pending until explicit Owner acceptance. |
