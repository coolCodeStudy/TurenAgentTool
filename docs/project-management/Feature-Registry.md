...
| Kline Agent | [`PRD-Kline-Agent.md`](../product/PRD-Kline-Agent.md) | ready | [`kline-agent-v1.md`](../techplans/kline-agent-v1.md) | implemented | local_verified | test_passed | pending | V1 command/CLI and Command Workbench routing exist for read-only daily/weekly/monthly Kline investigation, deterministic rules, fixture statistics, data-quality warnings, and degraded-provider reporting. The deployed cloud surface is still failed/major for `AT-2026-07-01-001` because the last deployed run returned provider-disabled/insufficient-evidence output without real bars. Local FutuD/OpenD remains forbidden; approved remote/cloud Futu is allowed when configured, and Development commit `0fd67b2` adds a no-credential `yahoo_chart` daily OHLCV provider for common U.S. stocks plus normal Workbench execution so `/command` no longer forces degraded Kline runs. Local verification covers mocked Yahoo bars, router/degraded behavior, and Workbench Kline preview/execution safety. Remaining gaps: deploy this returned branch to `/command`, run cloud live-data acceptance for `K线调查 US.NVDA 5年 前复权`, AkShare/secondary fallback, durable bar/run cache schema, portfolio/watchlist scans, decision-card/weekly-review integration, and user acceptance. | Kline Feature Coordinator accepted `DQ-2026-07-04-005` and dispatched Release/Deploy owner in `DQ-2026-07-04-006` for commit `0fd67b2` on `http://47.84.190.191:8010/command`; after deploy return, move `AT-2026-07-01-001` to `needs_retest` and route Acceptance Testing for a real-bar report before asking for user acceptance. |
