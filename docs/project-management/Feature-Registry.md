...
| Kline Agent | [`PRD-Kline-Agent.md`](../product/PRD-Kline-Agent.md) | ready | [`kline-agent-v1.md`](../techplans/kline-agent-v1.md) | implemented | deployed | needs_review | pending | V1 command/CLI and Command Workbench routing exist for read-only daily/weekly/monthly Kline investigation, deterministic rules, fixture statistics, data-quality warnings, and degraded-provider reporting. Local FutuD/OpenD remains forbidden; approved remote/cloud Futu is allowed when configured, and Development commit `0fd67b2` adds a no-credential `yahoo_chart` daily OHLCV provider for common U.S. stocks plus normal Workbench execution so `/command` no longer forces degraded Kline runs. The cloud release has repeatedly lost this commit during cross-feature deploys: Kline passed in event #313 after Ops #42, failed in event #333 after cross-feature deploy #43 at `f19cb9494594`, passed again in event #337 after reconciled Ops #44 at `5a941d3`, and then failed again in the user's fresh browser event #402 after cloud `系统总览` reported latest deploy #48 quick commit `880b741393ad`. Current browser behavior is `Provider: unavailable` with `KLINE_PROVIDER=disabled`, no bar count, and no daily/weekly/monthly statistics. Remaining gaps: Global PM release coordination to produce a combined deploy ref that does not drop Kline live-provider commit `0fd67b2`, rerun cloud acceptance, AkShare/secondary fallback, durable bar/run cache schema, portfolio/watchlist scans, decision-card/weekly-review integration, and user acceptance. | Escalated to Global PM in `DQ-2026-07-04-010` as a cross-feature/global deploy conflict. Do not ask for user acceptance until a combined release is deployed and the browser command returns real `yahoo_chart` bars again. |
