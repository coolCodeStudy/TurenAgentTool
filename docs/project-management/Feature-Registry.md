...
| Weekly review generator | [`PRD-每周复盘.md`](../product/PRD-每周复盘.md) | ready | [`weekly-review.md`](../techplans/weekly-review.md) | partially_implemented | local_verified | fixture_verified | rejected | Core weekly review generation, persistence, command path, and web surface exist. 2026-06-28 source-completeness engineering branch `codex/weekly-review-source-completeness` updates the technical plan, adds a Futu index-bar provider, adds source-status semantics for `ok`/`partial`/`checked_empty`/`missing`/`provider_unavailable`/`source_blocked`, connects official-source and local knowledge evidence into the story contract, and updates the Web page to render index and evidence-backed story sections instead of fixed missing-source copy. Local syntax and fixture checks passed; full smoke was limited by local Futu/OpenD being unavailable. Cloud deployment and provider-available verification are still pending, so `AT-2026-06-28-001` remains failed until the Coordinator integrates/deploys and dispatches retest. | Coordinator Return Gate should inspect and integrate `codex/weekly-review-source-completeness`, deploy the integrated ref with the weekly-review web surface, run cloud source/provider verification, then move `AT-2026-06-28-001` to `needs_retest` and dispatch Acceptance Testing if cloud evidence is adequate. |
