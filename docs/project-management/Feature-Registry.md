...
| Delivery coordination system | [`Delivery-Coordinator-Protocol.md`](../product/Delivery-Coordinator-Protocol.md) | ready | not_applicable | not_applicable | local_verified | test_passed | pending | Coordinator protocol, feature-specific audit, handoff packet generation, and delivery audit script are in place; future automation can turn frequent coordinator actions into skills or commands. | Use `python3 scripts/audit_delivery_state.py`, `--feature`, and `--handoff-packet` for delivery coordination; refine checks as new recurring gaps appear. |
