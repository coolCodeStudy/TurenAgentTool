...
| Daily market brief | [`PRD-Daily-Market-Brief.md`](../product/PRD-Daily-Market-Brief.md) | ready | [`daily-market-brief.md`](../techplans/daily-market-brief.md) | partially_implemented | local_verified | test_passed | pending | P0 command surface, `daily_market_brief` persistence helpers, market/date idempotency, scheduler-ready module entrypoint, CN/HK/US index generation, Simplified Chinese Markdown rendering, source-status metadata, fixture-backed verification, explicit capital-flow degraded states, weekend no-session handling, and command retrieval for latest/specified market/date are implemented. Acceptance-failure fix on `codex/daily-market-brief-acceptance-fix-07df` addresses failed retest `AT-2026-06-30-002`: command fixture mode now supplies deterministic index bars for CN/HK/US, user-facing `数据状态` renders product-language degraded copy without raw Yahoo/SSL/internal exception text, and legacy `review_reports.report_key` schema/upsert behavior is market-aware. Verification evidence: `.venv/bin/python -m unittest tests.test_daily_market_brief`; `.venv/bin/python -m py_compile investment_knowledge_mcp/daily_market_brief.py investment_knowledge_mcp/command_router.py investment_knowledge_mcp/repository.py investment_knowledge_mcp/market_data_provider.py`; `POSTGRES_PORT=55433 .venv/bin/python scripts/ikg.py` checks for CN fixture generation/retrieval/latest/rerun reusing `#130`, HK fixture `#131`, US fixture `#132`, weekend CN no-session `#133`, and forced degraded command-router output without raw provider text. Independent Acceptance retest on 2026-06-30 SGT passed `AT-2026-06-30-002` on coordinator-integrated commit `65aafba`: command checks regenerated/retrieved CN/HK/US fixtures, CN rerun reused `#130`, DB inspection showed market-aware CN/HK/US keys and no duplicate market/date rows, weekend CN recorded `no_session`, forced degraded command-router output hid raw Yahoo/SSL/DB internals, and command errors were user-readable. Known live-data gap remains: current configured providers do not supply full-market sector/industry rankings, liquid common-stock gainer universes, or explicit capital-flow metrics; live generation may degrade, but it must not hide gaps, invent rows, expose raw internals, or break cross-market storage. | Acceptance testing passed; user acceptance remains pending. Feature Coordinator should apply the Acceptance Return Gate and ask the user for Daily Market Brief P0 acceptance only with the known live-provider limitations disclosed. |
...
- `docs/techplans/daily-market-brief.md`
