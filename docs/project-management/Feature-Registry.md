...
| Stock valuation research | [`PRD-Stock-Valuation-Research.md`](../product/PRD-Stock-Valuation-Research.md) | ready | [`stock-valuation-research.md`](../techplans/stock-valuation-research.md) | implemented | deployed | deploy_verified | needs_reacceptance | P0 single-stock command flow and P0.1 provider-backed packet are implemented, cloud-deployed, independently acceptance-tested, and user accepted on 2026-07-04 SGT; do not reopen P0/P0.1. P0.2 product addendum from Product commit `3dbdf3f` is implemented through combined release `origin/codex/release-access-kline-stock-valuation-p02` at `d13f32023c3cc0ba0d587affd699835ecc74b159`, preserving Command Workbench access setup and Kline Workbench behavior while adding readable currency/percent/multiple formatting, provider-gap taxonomy for partial Yahoo failures, negative/meaningless PE/FCF-yield/EV-FCF labels with raw values preserved in artifacts, market-implied bridge lines, and frame-fit ranking against current market value. Coordinator full-deployed the combined ref through Ops event `#47` after quick deploy `#46` served a stale 17-action catalog. Independent acceptance found visible cloud Workbench behavior passed, including action catalog, aliases, readable values, negative-multiple labels, stable partial-provider-gap copy, market-implied bridge, frame-fit ranking, no advice, no formal user insight, and token secrecy. Remaining deployed P0.2 blocker is black-box verification of saved artifact raw-value preservation: Workbench exposes the rendered card and artifact path, but public artifact URL probes returned 404. Development return `DQ-2026-07-04-018` adds a bounded `valuation artifact evidence US.INTC` command on branch `codex/stock-valuation-coordinator-dispatch`; it still needs Coordinator Return Gate, deploy, and independent retest before user acceptance. Peer multiples, analyst estimates, normalized valuation tables, async refresh, and user-confirmed valuation-case workflow remain later follow-up scope. | Next owner is Stock valuation Feature Coordinator: inspect and integrate or reject the returned `DQ-2026-07-04-018` branch, deploy the accepted ref, move `AT-2026-07-04-002` to `needs_retest`, and rerun only the blocked artifact-preservation evidence check plus a light token-secrecy scan. Do not ask the user for P0.2 final acceptance while the row is blocked. |
| Kline Agent | [`PRD-Kline-Agent.md`](../product/PRD-Kline-Agent.md) | ready | [`kline-agent-v1.md`](../techplans/kline-agent-v1.md) | implemented | local_verified | test_passed | pending | V1 command/CLI path is implemented for single-stock read-only daily/weekly/monthly Kline investigation with deterministic rules, Futu provider abstraction, fixture verification, data-quality warnings, sample statistics, and insufficient-evidence output. Independent acceptance row `AT-2026-07-01-001` is blocked for full live-provider acceptance; coordinator follow-up cleared the missing worktree `.venv`, `futu-api`, and `scripts/ikg.py` DB-wrapper blockers. User clarified on 2026-07-03 that local FutuD/OpenD must never be used and has been deleted locally, so local acceptance can only cover fixture/degraded-provider behavior until a non-Futu provider path exists. User browser check on 2026-07-04 exposed a Command Workbench Web Preview gap: `/command` did not register Kline as a website action. Coordinator branch `codex/kline-agent-coordinator-return-gate` now adds the read-only Workbench `kline_investigation` action and exact-command parser tests; cloud merge/deploy is still required before browser retest. Remaining gaps: deploy/browser retest, live non-Futu provider validation, AkShare/secondary fallback, durable bar/run cache schema, portfolio/watchlist scans, decision-card/weekly-review integration, and user acceptance. | Request merge/deploy permission for the coordinator branch, deploy the `/command` Workbench Kline action, then rerun `AT-2026-07-01-001` browser degraded-scope retest. For full live K-line acceptance, create/approve a non-Futu provider path and rerun against at least one supported live single-stock Kline report. Ask the user for acceptance only if the deployed browser degraded-scope retest passes or the user explicitly accepts degraded-only scope. |
