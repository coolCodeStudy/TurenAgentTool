...
| Frontend experience system | [`PRD-Frontend-Experience-System.md`](../product/PRD-Frontend-Experience-System.md) | ready | [`frontend-experience-system.md`](../techplans/frontend-experience-system.md) | ready | not_started | doc_reference | not_required | Product PRD chooses the recommended two-surface first slice: shared server-rendered app shell, primary navigation, and design-token contract for `/weekly-review` and `/command`. Technical plan defines one bounded Development pass with shared `frontend_shell.py`, renderer refactors, smoke assertions, desktop/mobile evidence, Release Reviewer before deploy, and Acceptance Reviewer after deploy. Daily Market Brief remains planning-only with no route/package/renderer and must not get a fake nav affordance. Implementation, deployment, independent acceptance testing, and user acceptance are still missing. | Feature Coordinator should apply Return Gate to Development technical-plan row `DQ-2026-07-05-003`, then dispatch Frontend Experience Reviewer before implementation handoff. If reviewer passes, dispatch Development implementation for the bounded shell slice. Release Reviewer is required before deploy because both `/weekly-review` and `/command` change in one release; Acceptance Reviewer is required after deployment. |
