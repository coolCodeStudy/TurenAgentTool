...
| Command Workbench | [`PRD-command-workbench.md`](../product/PRD-command-workbench.md) | ready | [`command-workbench.md`](../techplans/command-workbench.md) | implemented | deployed | deploy_verified | pending | Bounded V1 is implemented for registry-backed parsing, preview, candidate selection, confirmation guards, `/command` UI, workbench APIs, and Level 1 decision-card execution. Cloud acceptance is served at `:8010/command` because `command-api` runs internally but `:8001` is not publicly reachable. Decision-ticket/history actions recover explicitly because only Level 1 decision cards exist today. Full smoke test was blocked by local PostgreSQL `localhost:55432` being unavailable. 2026-06-28 acceptance testing is blocked: `GET /command` and the action catalog endpoint are reachable, but every cloud parse/execute probe returns HTTP 503 `command workbench token is not configured`, so representative preview, execute, unsupported-action, and auth flows cannot pass. User acceptance is still pending; do not mark accepted until the user verifies the behavior after acceptance testing passes. | Engineering/Ops must configure the command workbench token for the cloud `:8010` service, redeploy or restart as needed, then move `AT-2026-06-25-002` to `needs_retest` and rerun full black-box acceptance. Revisit separate decision-ticket/history scope only after the deployed V1 workbench is acceptance-testable. |
