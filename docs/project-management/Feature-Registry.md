...
| Daily market brief | [`PRD-Daily-Market-Brief.md`](../product/PRD-Daily-Market-Brief.md) | draft | missing | missing | not_started | doc_reference | pending | Initial coordinator draft captures P0 intent for independent CN/HK/US post-close briefs, report persistence/retrieval options, scheduler/idempotency requirements, provider fallback rules, and acceptance criteria. Product decisions remain open for output surface, exact indexes, sector taxonomy/providers, capital-flow definition, stock universe/liquidity filter, baseline windows, language/timezone, and delivery timing. No acceptance row yet because there is no implemented or user-facing surface to test. | Product Agent should refine the draft into a ready PRD or record defaults, then hand off to Engineering for `docs/techplans/daily-market-brief.md`. |
...
- Daily market brief: PRD draft exists; Product must resolve output surface, provider/taxonomy, capital-flow, stock universe, baseline, language/timezone, and delivery timing decisions before technical planning.
