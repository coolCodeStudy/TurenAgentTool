...
| Command Workbench | [`PRD-command-workbench.md`](../product/PRD-command-workbench.md) | ready | [`command-workbench.md`](../techplans/command-workbench.md) | implemented | deployed | deploy_verified | pending | Bounded V1 is implemented for registry-backed parsing, preview, candidate selection, confirmation guards, `/command` UI, workbench APIs, and Level 1 decision-card execution. Cloud acceptance is served at `:8010/command` because `command-api` runs internally but `:8001` is not publicly reachable. Decision-ticket/history actions recover explicitly because only Level 1 decision cards exist today. Full smoke test was blocked by local PostgreSQL `localhost:55432` being unavailable. 2026-06-28 acceptance testing found `GET /command` and the action catalog endpoint reachable but parse/execute blocked by HTTP 503 `command workbench token is not configured`; Development/Ops fixed the container env by passing `COMMAND_API_TOKEN` to `weekly-review-web`, deployed branch `codex/command-workbench-token-config` commit `0c22a05` with GitHub Actions run `28325425811`, and verified cloud parse now returns HTTP 401 `unauthorized` without a token instead of configuration-missing 503. User acceptance is still pending; do not mark accepted until the user verifies the behavior after acceptance testing passes. | Dispatch Acceptance Testing to rerun `AT-2026-06-25-002` against `http://47.84.190.191:8010/command` with the configured token, covering token-bearing parse/execute previews, missing/invalid-token auth behavior, unsupported-action recovery, and browser evidence. Revisit separate decision-ticket/history scope only after the deployed V1 workbench is acceptance-tested. |
