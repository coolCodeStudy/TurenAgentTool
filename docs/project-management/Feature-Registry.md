...
| Weekly review generator | [`PRD-每周复盘.md`](../product/PRD-每周复盘.md) | ready | [`weekly-review.md`](../techplans/weekly-review.md) | partially_implemented | local_verified | fixture_verified | rejected | Core weekly review generation, persistence, command path, and web surface exist. 2026-06-28 source-completeness engineering branch `origin/codex/weekly-review-source-completeness` at `ce36764` updates the technical plan, adds a Futu index-bar provider, adds source-status semantics for `ok`/`partial`/`checked_empty`/`missing`/`provider_unavailable`/`source_blocked`, connects official-source and local knowledge evidence into the story contract, and updates the Web page to render index and evidence-backed story sections instead of fixed missing-source copy. Coordinator Return Gate accepted the branch and cherry-picked it locally as integration commit `80da965` on `codex/weekly-review-source-completeness-integration`; coordinator state is recorded on the same local branch. Local syntax, fixture, Web HTML, and diff checks passed; full smoke was limited by local PostgreSQL/Futu/OpenD availability. Cloud deployment and provider-available verification are still pending, so `AT-2026-06-28-001` remains failed. | Push/deploy is blocked in the coordinator environment by missing GitHub HTTPS credentials. An authenticated integration session should push or cherry-pick the local integration branch commits to `main`, deploy the Weekly Review Web surface, run cloud source/provider verification, then move `AT-2026-06-28-001` to `needs_retest` and dispatch Acceptance Testing if cloud evidence is adequate. |
