...
| Stock valuation research | [`PRD-Stock-Valuation-Research.md`](../product/PRD-Stock-Valuation-Research.md) | ready | missing | missing | not_started | doc_reference | pending | PRD is ready for technical planning with P0 scoped to single-stock command retrieval plus saved valuation artifact, five internal core frames with 1-3 user-facing frames, explicit source/citation rules, and deterministic degraded behavior. No technical plan or implementation exists yet. | Engineering: create the P0 technical plan, then implement single-stock valuation research command/artifact flow. |
