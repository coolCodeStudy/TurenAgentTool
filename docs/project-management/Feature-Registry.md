...
| Daily market brief | [`PRD-Daily-Market-Brief.md`](../product/PRD-Daily-Market-Brief.md) | ready | [`daily-market-brief.md`](../techplans/daily-market-brief.md) | partially_implemented | deployed | deploy_verified | pending | P0 command surface, `daily_market_brief` persistence helpers, market/date idempotency, scheduler-ready module entrypoint, CN/HK/US index generation, Simplified Chinese Markdown rendering, source-status metadata, fixture-backed verification, explicit capital-flow degraded states, weekend no-session handling, and command retrieval for latest/specified market/date are implemented. Acceptance-failure fix on `codex/daily-market-brief-acceptance-fix-07df` addresses failed retest `AT-2026-06-30-002`: command fixture mode now supplies deterministic index bars for CN/HK/US, user-facing `数据状态` renders product-language degraded copy without raw Yahoo/SSL/internal exception text, and legacy `review_reports.report_key` schema/upsert behavior is market-aware. Independent Acceptance retest on 2026-06-30 SGT passed the command surface in `AT-2026-06-30-002`. User correction on 2026-07-01 made CLI-only acceptance insufficient, so the existing Web service now exposes `/daily-market-brief`, `GET /api/daily-market-brief`, and `POST /api/daily-market-brief/generate`; local tests verify page/API renderability. Independent Web Acceptance retest `AT-2026-07-01-001` passed on 2026-07-01 SGT against local weekly-review Web service at coordinator commit `54f9031`, with minor UX observations for unauthenticated error copy and narrow-screen timestamp wrapping. Full cloud deploy run `28670512010` succeeded on 2026-07-03 SGT from `codex/daily-market-brief-coordinator`; cloud verification returned 200 for `/health`, `/weekly-review`, and `/daily-market-brief`, and HTML checks confirmed the Daily Market Brief page, API hooks, CN tab, Markdown section, and Weekly Review navigation link. Known live-data gap remains: current configured providers do not supply full-market sector/industry rankings, liquid common-stock gainer universes, or explicit capital-flow metrics; live generation may degrade, but it must not hide gaps, invent rows, expose raw internals, or break cross-market storage. | Ready for user acceptance through `http://47.84.190.191:8010/daily-market-brief`. Do not ask the user to accept via CLI, and do not mark user acceptance accepted. |
