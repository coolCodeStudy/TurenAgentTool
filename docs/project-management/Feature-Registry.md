...
| Kline Agent | [`PRD-Kline-Agent.md`](../product/PRD-Kline-Agent.md) | ready | [`kline-agent-v1.md`](../techplans/kline-agent-v1.md) | implemented | local_verified | test_passed | pending | V1 command/CLI path is implemented for single-stock read-only daily/weekly/monthly Kline investigation with deterministic rules, Futu provider abstraction, fixture verification, data-quality warnings, sample statistics, and insufficient-evidence output. Independent acceptance row `AT-2026-07-01-001` is blocked for full live-provider acceptance; coordinator follow-up cleared the missing worktree `.venv`, `futu-api`, and `scripts/ikg.py` DB-wrapper blockers by installing dependencies and running the command path with `POSTGRES_PORT=55433 FUTU_OPEND_PORT=11111`. Remaining live blocker is Futu OpenD operational state: local config says API port `11111`, but no listener is active and the command returns provider limitation `Futu OpenD is not reachable at 127.0.0.1:11111`. Fixture and provider-unavailable behavior passed. Remaining gaps: live Futu/OpenD provider validation, AkShare/secondary fallback, durable bar/run cache schema, portfolio/watchlist scans, decision-card/weekly-review integration, and user acceptance. | Start/login local Futu OpenD until `127.0.0.1:11111` listens, rerun `AT-2026-07-01-001` as `needs_retest` against at least one supported live single-stock Kline report, then ask the user for acceptance only if the live-provider retest passes. Plan V2 provider/cache/workflow integrations separately. |
