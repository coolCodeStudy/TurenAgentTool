...
| DQ-2026-06-28-006 | Command Workbench | Development/Ops Agent | returned | branch `origin/codex/command-workbench-token-config`; commit `0c22a05`; deploy run `28325425811` | Coordinator Return Gate for `DQ-2026-06-28-005`; Acceptance Queue row `AT-2026-06-25-002` blocked/blocker; returned acceptance evidence `/tmp/command-workbench-acceptance-20260628.json` and `/tmp/command-workbench-initial-20260628.png` | Configure the cloud public `:8010` service with the required command workbench token, restart or redeploy as needed, verify parse/execute no longer return 503, and move `AT-2026-06-25-002` to `needs_retest` when ready. | Returned by Development/Ops: `weekly-review-web` now receives `COMMAND_API_TOKEN`, cloud deploy succeeded, and tokenless parse returns HTTP 401 instead of configuration-missing 503. Coordinator should apply the Return Gate, then dispatch Acceptance Testing for a full token-bearing black-box retest. Do not mark user acceptance accepted. |
