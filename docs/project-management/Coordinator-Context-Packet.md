...
- Current state: Product-ready PRD `91b741b` plus the approved bilingual presentation addendum; technical plan `implementation_complete_pending_release`; bilingual Development candidate `68aae6f` is locally verified and pushed. The candidate is based on authoritative `origin/main@0bc4539f33428419c64e1542be991606f134716c` and requires one serialized quick release. The independent protected Stock P0 evidence blocker remains unchanged.
...
- Next owner: Feature Coordinator self-deploy for integrated candidate `68aae6f`; after smoke, independent Quality & Acceptance Lead owns `AT-2026-07-19-001` retest.
- Next handoff: `needs_deploy`: integrate onto latest main, run the classifier, record one shared serialized quick Deploy Intent, smoke `http://47.84.190.191:8010/command`, then dispatch the same AT row. GitHub `E2E_PROTECTED_ACCESS_TOKEN` remains unnecessary for public/local-first acceptance; the protected P0 blocker is retained separately.
- Deploy needed: Yes; the bilingual renderer changes are user-facing and not in the currently deployed runtime.
- Deploy decision: Pending classifier on the integrated ref; expected `quick`/`targeted_quick` using only the shared workflow and existing service targets. Do not alter ingress/Compose/public-port wiring.
