...
| AT-2026-06-25-002 | Command Workbench | `http://47.84.190.191:8010/command` | blocked | major | Retest by Acceptance Testing Agent on 2026-06-28 SGT. Evidence JSON: `/tmp/command-workbench-acceptance-blocked-20260628.json`; browser screenshots: `/tmp/command-workbench-desktop-initial-20260628.png`, `/tmp/command-workbench-mobile-initial-20260628.png`. Cloud checks showed `/command` 200 with the Command Workbench page, `/api/command-workbench/actions` 200 with 16 registered actions across Decision, Portfolio, Research, System, and Weekly Review, tokenless parse 401 `unauthorized`, invalid-token parse 401 `unauthorized`, and invalid-token execute 401 `unauthorized`. 2026-06-29 product decision: P0 may use one fixed private owner/test token configured outside the repo and stored only in browser localStorage by the `/command` page. | No valid configured cloud command token was available to this acceptance-testing session through approved secure sources. Worktree `COMMAND_API_TOKEN` was absent, the main-workspace `.env` has the key but no value, the browser token field loaded empty, and a filename-only local sweep found no separate command token file. Because token-bearing parse/preview/execute could not be run, the required representative commands, safe executes, confirmation guards, unsupported execute guard, catalog form parse, and recent-history validation remain untested. The current `/command` UI also needs clearer missing/invalid-token recovery copy before user-facing acceptance. | Engineering should first make token entry/recovery low-friction and clear, then the coordinator or credential owner must provide an approved secret path/session for the fixed private product token and rerun full black-box acceptance. Do not ask for user acceptance while this row is blocked. |
