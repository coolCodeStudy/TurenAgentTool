...
| AT-2026-06-25-002 | Command Workbench | `http://47.84.190.191:8010/command` | blocked | blocker | Acceptance Testing Agent cloud black-box run on 2026-06-28 SGT. Evidence JSON: `/tmp/command-workbench-acceptance-20260628.json`; browser screenshot: `/tmp/command-workbench-initial-20260628.png`. `GET /command` returned 200 and the action catalog endpoint returned 200, but every parse and execute probe returned HTTP 503 with `command workbench token is not configured`: `决策 英特尔`, `决策 阿里`, `本周复盘`, `系统状态`, unsupported trading/free text, catalog decision form, missing-auth execute, and invalid-auth execute. | Blocked before representative preview or execute flows could be accepted. The public web service is reachable, but its command-workbench auth configuration is missing, and the user-facing API exposes an internal configuration error instead of the expected preview, unsupported-action recovery, or auth response. | Engineering/Ops must configure the command workbench token on the cloud `:8010` service, redeploy or restart as needed, then move this row to `needs_retest` and rerun the full black-box acceptance scope. Do not ask the user for acceptance while this row is blocked. |
