...
| AT-2026-06-25-002 | Command Workbench | `http://47.84.190.191:8010/command` | needs_retest | major | 2026-06-29 Acceptance retest using the approved local token source was blocked because the current cloud runtime returned HTTP 503 `command workbench token is not configured`; evidence JSON: `/tmp/command-workbench-acceptance-blocked-20260629.json`; browser screenshots: `/tmp/command-workbench-desktop-blocked-20260629.png`, `/tmp/command-workbench-mobile-blocked-20260629.png`. Development/Ops root cause: a later full deploy from `main` recreated `weekly-review-web` from a ref without `COMMAND_API_TOKEN` wiring, overwriting the briefly working branch deploy. Development/Ops restored runtime token wiring with full deploy run `28384150041` on runtime code commit `6f0a207` and quick deploy run `28384624938` on final commit `55bd671`. Coordinator Return Gate verification after the Ops return: `/command` 200, missing-token parse 401 with recovery copy, invalid-token parse 401 with recovery copy, valid-token parse `决策 英特尔` 200 with preview, and valid-token execute `系统状态` 200. Token values were not recorded in docs, evidence JSON, screenshots, or handoffs. | Token-bearing parse/preview and safe execute are available again for independent acceptance retest through the approved local secret path supplied to the Acceptance Testing Agent. Required scope still to verify independently: representative commands, safe executes, confirmation guards, unsupported execute guard, catalog form parse, recent-history validation, browser desktop/mobile evidence, user-facing error handling, and token secrecy. | Acceptance Testing Agent thread `019f1413-f043-73c1-aba8-3d749c2eb05a` is dispatched to rerun full black-box acceptance against `http://47.84.190.191:8010/command`. Do not ask for user acceptance until this row passes. |
