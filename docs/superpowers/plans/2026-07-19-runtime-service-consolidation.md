...
**Implementation status (2026-07-19):** Completed and deployed on authoritative `main@86652e4` by full deploy run `29660355957`. Baseline run `29652847798` recorded nine application containers at approximately 431.16 MiB. Diagnostics run `29660549295` records five application containers at approximately 278.36 MiB plus PostgreSQL, reducing application-container memory by approximately 152.80 MiB (35.4%) and raising host `MemAvailable` by approximately 121.5 MiB. Three schedules now share `scheduler-host`, history is an on-demand supervised child, standalone `command-api` is retired behind the gateway's port-8001 compatibility mapping, and migration/rollback contracts are regression-tested. DingTalk HTTP instrumentation is live, but removal remains intentionally unadmitted pending a clean 14-day sanitized observation window from this deployment plus callback/audio inventory. Delivery evidence is closed in `DQ-2026-07-19-001`.
