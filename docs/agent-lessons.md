...
- For a user-triggered operation that may take longer than ordinary UI feedback, add a browser regression that holds the request immediately after the click. Assert that progress is visible and competing controls are disabled before asserting the eventual result; an eventual-success-only test can miss an apparently inert interaction.
...
- When the Owner gives continuing delivery authorization, treat ordinary product and technical choices, commits, pushes, shared-workflow releases, verification, and acceptance routing as pre-approved. Do not reopen routine confirmations; surface only a genuine new authority need, destructive-scope decision, external account or spending decision, real cross-feature conflict, or final user acceptance.
...
- A GitHub runner can plan with the target deployment contract while the independent Ops API executes an older installed contract. Compare the installed immutable control-plane ref to the target by control-plane-file lineage: install the target when that lineage contains control-plane changes, but allow later documentation/test-only descendants without reinstalling an identical control plane. A successful runner-side plan alone is not compatibility evidence.
- When adding a known GitHub Actions workflow, add an explicit `scripts/deploy_contract.py` path rule and a `tests/test_deploy_change_classifier.py` regression case in the same change. Unknown workflow paths are intentionally rejected, so omitting either declaration can block an otherwise unrelated application release.
...
- During a runtime topology migration, wait boundedly for a replacement's first internal readiness snapshot, and evaluate rollback health against the restored release's service contract. Applying the candidate contract to a legacy topology can falsely mark a successful restoration as failed and trigger unnecessary lockout.
...
- For Weekly Review Web acceptance, verify the live report after the latest deployed `main` ref, not just after the first full deploy of a code SHA. Later quick/push deploys can leave the user-facing page serving old missing-source output until a latest-main full deploy and force refresh are completed.
...
- Before presenting a cloud-served feature for user acceptance, compare the public health endpoint's active application SHA with the exact ref covered by deployment and acceptance evidence. A successful historical deployment does not prove that a later cross-feature release has not replaced the active application.
