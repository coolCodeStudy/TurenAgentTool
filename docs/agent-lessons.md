...
- After an approved task-branch push with product-affecting changes, the default next step is merge plus deploy, not stopping at the push summary. Product-affecting changes include code, configuration, schema, frontend assets, service behavior, or runtime data-flow changes. Pure documentation, tech plan, PRD, or comment-only changes should be committed and pushed, but should not be deployed unless the files are served by the product or the user explicitly asks for deployment.
