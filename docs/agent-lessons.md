...
- The Global Project Manager should not become the default watcher for every feature's child threads. Feature-level user decisions belong to the Feature Coordinator, which asks the user and continues the feature flow. Use the Global PM for portfolio health, escalation, stale-coordinator recovery, cross-feature conflict resolution, merge/deploy permission, and delivery-system process defects; normal child-thread Return Gate ownership stays with the feature coordinator.
