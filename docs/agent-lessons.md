...
- When the user asks to "做完" or otherwise wants an end-to-end feature delivery, treat the default finish line as local implementation plus narrow verification, commit, push, and, for cloud-served/user-visible surfaces, the appropriate deploy and cloud verification. Still state the target branch, deploy mode, touched services, database target, ports, and external integrations before remote/deploy actions.
