...
- A GitHub runner can plan with the target deployment contract while the independent Ops API executes an older installed contract. When the diff changes control-plane files, compare the installed immutable control-plane ref with the exact target, install that target first, and only then retry the single business deployment channel; a successful runner-side plan is not compatibility evidence.
