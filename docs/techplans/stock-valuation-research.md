...
| 3 | 1-3 selected frames plus versioned saved artifact | Packet/card/save-load tests | **needs_review** — fresh re-review reproduced two different processes both succeeding in a same-stock/same-second race; inter-process collision exclusion around both atomic replacements is active. |
...
| 7 | SEC, HKEX/company, DART/FSS/company source plans | US/HK/KR injected provider fixtures and attempt-order tests | **needs_review** — one-bundle fan-out remains correct, but fresh re-review found timeout/saturation/submission failure is falsely labeled `complete_missing`; a shared typed bounded outcome is active. |
| 8 | Shared market snapshot pool, provider/currency/time/freshness, attempts before degradation | Pool fallback/partial/unavailable tests | **needs_review** — the six-second/two-worker bound passes, but timeout/saturation must project safe failed/retry guidance rather than definitive complete-missing discovery. |
...
| 10 | Separate facts/assumptions/calculations/interpretation/watch layers and provenance | Packet/card section tests | **needs_review** — artifact layers are present, but fresh re-review found the concise card still lacks separately visible Facts and Calculations sections with safe source/period and input references. |
| 11 | Candidate/manual peer status only | Missing/stale/candidate peer confidence tests | **needs_review** — exact build-time valuation-case matching passes, but stored boolean confirmation can be self-asserted and missing peer evidence still leaves Comparable Multiples at medium confidence; fail-closed confirmation provenance and peer-gap caps are active. |
| 12 | Named financial/market/both-missing degraded states | Three degraded-state fixtures | **needs_review** — state matrices pass, but bounded provider failures are relabeled complete-missing; typed failed/timeout/saturated recovery semantics are active. |
| 13 | Stock-scoped allow-list evidence projection | Evidence and path-like rejection/leakage tests plus cloud probe | **needs_review** — fresh re-review reproduced `RecursionError` from deeply nested latest JSON and evidence input, plus tampered confirmation acceptance; byte/depth/container limits and total boundary catches are active. |
