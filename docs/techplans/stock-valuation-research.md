...
After Development and whole-branch review pass, the Feature Coordinator records Deploy Intent with the exact pushed integrated ref (current candidate `46639d2`, including Frontend runtime ref `017d91c6cc8ad613ab07a2b9093bb7b32850abba`), classifier-selected mode, affected `weekly-review-web`/gateway service set, cloud URL `http://47.84.190.191:8010/command`, and this coordinator task as watch owner. Deployment uses only the serialized GitHub Actions/Ops API workflow and the current control-plane version gate.
