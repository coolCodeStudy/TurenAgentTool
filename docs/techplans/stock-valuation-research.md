...
After Development and whole-branch review pass, the Feature Coordinator records Deploy Intent with the immutable Stock runtime ref (`main@48930c1e56ea377e7fb65fb44279e172e36b054d`, contained in the latest serialized shared-service runtime `main@88df3fd7e8eeeebc43e1758b5c3471a41993e593`, run `29847396363`, event `1784650393858`; runtime integration parent `46639d2` and Frontend runtime ref `017d91c6cc8ad613ab07a2b9093bb7b32850abba`), affected `weekly-review-web`/gateway service set, cloud URL `http://47.84.190.191:8010/command`, and this coordinator task as watch owner. Deployment uses only the serialized GitHub Actions/Ops API workflow and the current control-plane version gate; latest docs/classifier-only descendants are `no_deploy`, and independent protected acceptance remains the next gate. The local-first policy removes GitHub protected-Chromium setup as a prerequisite for public acceptance but does not remove the authenticated Command Workbench boundary required by this P0.
