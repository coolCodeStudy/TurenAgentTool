...
After Development and whole-branch review pass, the Feature Coordinator records Deploy Intent with the exact pushed integrated ref (current candidate `25cc521`), classifier-selected mode, affected `weekly-review-web`/gateway service set, cloud URL `http://47.84.190.191:8010/command`, and this coordinator task as watch owner. Deployment uses only the serialized GitHub Actions/Ops API workflow and the current control-plane version gate.
