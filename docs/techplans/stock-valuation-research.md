...
- `valuation 建滔积层板 HK`, `valuation 建滔積層板 HK`, `valuation Kingboard Laminates HK`, `valuation HK.01888`, `valuation 1888 HK`: create and save a P0.3 non-US valuation artifact for Kingboard Laminates with normalized target `HK.01888`, provider ticker `1888.HK`, and explicit `HKD` labeling while preserving readable `HK$` formatting.
...
| `valuation 建滔积层板 HK` / `valuation 建滔積層板 HK` / `valuation Kingboard Laminates HK` | Resolves to Kingboard Laminates Holdings Limited, normalized `HK.01888`, provider ticker `1888.HK`, currency `HKD`, HKEXnews/company-report source attempt, fallback-fundamentals status, no raw diagnostics, and does not reroute supported P0.3 valuation fixtures to stock-profile bootstrap when a profile row is absent. | `tests.test_stock_valuation.StockValuationTests.test_p0_3_hk_alias_maps_to_kingboard_and_evidence_preserves_mapping`; `tests.test_stock_valuation.StockValuationTests.test_p0_3_hk_and_kr_command_forms_normalize_in_workbench_and_router` |
...
No service startup, database migration, external credential, or new provider account is required for P0.3 local command verification. Because the accepted user surface is the cloud Command Workbench at `http://47.84.190.191:8010/command`, P0.3 still needs cloud deploy of the combined release ref and independent cloud-IP retest using the private token route before user acceptance. Direct deploy of the stock-only coordinator branch is blocked because it would clobber Kline Workbench behavior; use `origin/codex/release-kline-stock-valuation-p03` instead.
...
| P0.2 readable number formatting | accepted | `stock_valuation.py`, `tests/test_stock_valuation.py`, cloud evidence in `AT-2026-07-04-002` | Card uses compact currency, percent, per-share, and multiple displays; artifact preserves raw numeric values plus display metadata. User accepted P0.2 on 2026-07-05. |
| P0.2 provider-gap taxonomy | accepted | `stock_valuation.py`, `tests/test_stock_valuation.py`, cloud evidence in `AT-2026-07-04-002` | Market snapshot status can report `partial_provider_gap` when Yahoo price/market-cap facts exist but another Yahoo call fails. User accepted P0.2 on 2026-07-05. |
| P0.2 user-facing provider-gap copy sanitization | accepted | `stock_valuation.py`, `tests/test_stock_valuation.py`, cloud evidence in `AT-2026-07-04-002` | Card degraded-state lines use provider taxonomy summaries; acceptance retest verified raw HTTP diagnostics, exception fragments, URLs, auth-ish text, token values, local paths, and file-read content stay out of rendered cards and bounded evidence read-back. |
| P0.2 negative/meaningless ratio handling | accepted | `stock_valuation.py`, `tests/test_stock_valuation.py`, cloud evidence in `AT-2026-07-04-002` | Negative PE, FCF yield, and EV/FCF render as `not meaningful` with reasons while raw values remain in artifact diagnostics. User accepted P0.2 on 2026-07-05. |
| P0.2 market-implied bridge | accepted | `stock_valuation.py`, `tests/test_stock_valuation.py`, cloud evidence in `AT-2026-07-04-002` | Includes P/S and EV/sales anchors, required future FCF margin lines for negative FCF, cycle-normalized earnings placeholder for negative earnings, and no target-price precision. User accepted P0.2 on 2026-07-05. |
| P0.2 frame fit ranking | accepted | `stock_valuation.py`, `tests/test_stock_valuation.py`, cloud evidence in `AT-2026-07-04-002` | Selected frames are ranked by `fit_to_current_market_value` with assumptions, must-become-true items, gaps, and confidence. User accepted P0.2 on 2026-07-05. |
| P0.2 black-box-safe artifact evidence path | accepted | `stock_valuation.py`, `command_router.py`, `command_workbench.py`, `tests/test_stock_valuation.py`, cloud evidence in `AT-2026-07-04-002` | `valuation artifact evidence US.INTC` returns bounded JSON evidence from the latest stock valuation artifact; the final copy-fix acceptance retest passed raw/display/meaningfulness/provider/bridge/frame-fit checks and token/secrets/path safety checks. |
...
| P0.3 HKD/KRW currency behavior | local_verified | `stock_valuation.py`, `valuation_data_provider.py`, `tests/test_stock_valuation.py` | Supported fixtures render `KRW` and `HK$` values instead of `unknown currency`; HK fixture card rows also include an explicit `HKD` label to avoid ambiguity. |
