...
- [x] Run the focused new suite: `.venv/bin/python -m unittest tests.test_stock_valuation tests.test_command_workbench tests.test_app_gateway tests.test_data_source_contracts tests.test_data_source_pool -v`; whole-branch fix exit 0, `Ran 144 tests in 0.556s`, `OK`.
- [x] Run the current-main preservation suite: `.venv/bin/python -m unittest tests.test_command_workbench tests.test_app_gateway tests.test_data_source_pool tests.test_data_source_contracts tests.test_data_source_market_bars tests.test_data_source_market_activity`; whole-branch fix exit 0, `Ran 88 tests in 0.039s`, `OK` (pre-feature baseline: 75).
- [x] Run the planner's broader gateway/access/frontend/provider suite: `.venv/bin/python -m unittest tests.test_command_workbench tests.test_app_gateway tests.test_command_http tests.test_web_experience tests.test_data_source_pool tests.test_data_source_contracts tests.test_data_source_market_bars tests.test_data_source_market_activity -v`; whole-branch fix exit 0, `Ran 128 tests in 0.560s`, `OK` (pre-feature baseline: 114).
...
| 3 | 1-3 selected frames plus versioned saved artifact | Packet/card/save-load tests | **verified** — `_write_packet()` uses validated/fsynced temporary files, stock-scoped synchronization, atomic replace, and explicit same-second collision handling; atomic/collision/interruption tests pass in `tests/test_stock_valuation.py`; whole-branch focused suite 144/144. |
...
| 7 | SEC, HKEX/company, DART/FSS/company source plans | US/HK/KR injected provider fixtures and attempt-order tests | **verified** — stored/public DART, HKEX, and eligible company-IR fixtures count regulator/company official families consistently; HK bundle fan-out calls `collect()` once and reuses complete-missing discovery; whole-branch focused suite 144/144. |
| 8 | Shared market snapshot pool, provider/currency/time/freshness, attempts before degradation | Pool fallback/partial/unavailable tests | **verified** — adapter freshness and aware fetch times survive packet/public projection; the declared official-provider ceiling is 6 seconds with bounded two-worker execution and safe timeout degradation below the 15-second Workbench abort; whole-branch focused suite 144/144. |
...
| 10 | Separate facts/assumptions/calculations/interpretation/watch layers and provenance | Packet/card section tests | **verified** — typed canonical projection/loader tests prove deterministic rule provenance, optional narrative unavailable state, frame semantics, bridge lines, freshness/source coverage, and distinct watch failures/triggers; whole-branch focused suite 144/144. |
| 11 | Candidate/manual peer status only | Missing/stale/candidate peer confidence tests | **verified** — unrelated confirmed stock records stay false; only an exact-target `stock_valuation_case.v1` entry can set confirmation, and absence is not a data/source gap; whole-branch focused suite 144/144. |
| 12 | Named financial/market/both-missing degraded states | Three degraded-state fixtures | **verified** — financial-only, market-only, both-missing, and stale cards name safe attempted families, missing categories, refresh recovery, and suppress unsupported bridge conclusions; whole-branch focused suite 144/144. |
