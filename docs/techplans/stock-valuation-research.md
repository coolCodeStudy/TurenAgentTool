...
| 2 | Five core plus three specialist-only method definitions | Method-library unit test and cloud methods command | **needs_review** — whole-branch review found the specialist label must be the exact Product name `Residual Income / ROE-PB`; fix and exact-name coverage are active. |
| 3 | 1-3 selected frames plus versioned saved artifact | Packet/card/save-load tests | **needs_review** — whole-branch review found timestamped/latest writes are non-atomic under concurrent same-stock requests; stock-scoped atomic persistence coverage is active. |
| 4 | Assumptions, supported market bridge, triggers, failures, confidence, freshness, gaps | Complete/degraded packet fixtures | **needs_review** — whole-branch review reproduced missing per-frame semantics, freshness/stale fields, bridge/source rendering, recovery categories, and rule provenance; semantic complete/stale/missing fixtures are active. |
...
| 7 | SEC, HKEX/company, DART/FSS/company source plans | US/HK/KR injected provider fixtures and attempt-order tests | **needs_review** — whole-branch review found DART/FSS/HKEX/company IR are omitted from the official-family count and the HK synchronous path repeats the official bundle; truthful count and one-collection coverage are active. |
| 8 | Shared market snapshot pool, provider/currency/time/freshness, attempts before degradation | Pool fallback/partial/unavailable tests | **needs_review** — whole-branch review found adapter freshness is dropped and no total provider budget protects the 15-second Workbench boundary; bounded single-collection/cache/degradation coverage is active. |
...
| 10 | Separate facts/assumptions/calculations/interpretation/watch layers and provenance | Packet/card section tests | **needs_review** — whole-branch review found heading-only coverage does not prove required provenance/source/freshness semantics; typed rule provenance and semantic card assertions are active. |
| 11 | Candidate/manual peer status only | Missing/stale/candidate peer confidence tests | **needs_review** — whole-branch review reproduced unrelated confirmed knowledge upgrading a valuation case; only explicit typed target-matched valuation-case confirmation may set this state. |
| 12 | Named financial/market/both-missing degraded states | Three degraded-state fixtures | **needs_review** — whole-branch review found recovery cards omit attempted source families and missing categories; financial-only, market-only, both-missing, and stale semantic fixtures are active. |
