...
3. The accountable data/licence operator must supply the approval-register facts and confirm deployed quote entitlements. Only then may the Feature Coordinator record that non-secret metadata in `FUTU_CROWDING_APPROVAL`, verify and deploy the enablement commit through the shared path, configure the approved runtime environment and activation switch, and rerun the same acceptance item. Do not create an enablement release before the external gate is satisfied.
