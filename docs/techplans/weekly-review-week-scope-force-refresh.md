...
> Status note (2026-06-18): This is the current accepted technical contract for the weekly review workbench. Older PRD or tech-plan wording about `refresh_draft`, compare views, finalized-report protection, status machines, or arbitrary `start/end` date ranges is historical context only.
...
Online diagnostics are available without generating or overwriting a weekly report:

```text
GET /api/weekly-review/diagnostics/indexes?week=2026-W25
周复盘指数诊断 2026-W25
```
