...
- Local owner access should use `scripts/setup_command_workbench_access.py`: generate or reuse a local token at `~/.config/turen-agent/command_workbench_token`, sync it to GitHub secret `COMMAND_API_TOKEN`, trigger the deploy workflow, verify cloud parse, and optionally open `/command` with a URL-fragment bootstrap that stores the token in browser localStorage without sending it to the server. Token rotation defaults to full deploy because the GitHub Actions full path rewrites the cloud `.env`; quick deploy only recreates services from the existing cloud env file.
