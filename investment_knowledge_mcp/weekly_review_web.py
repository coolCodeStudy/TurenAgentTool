...
class WeeklyReviewWebHandler(BaseHTTPRequestHandler):
    ...
    def _handle_daily_market_brief_generate(self, payload: dict[str, Any]) -> None:
        try:
            ...
            run_schema()
            result = build_daily_market_brief(market=market, market_date=market_date, save=True, use_fixture=use_fixture)
        except ValueError as exc:
            self._write_json(HTTPStatus.BAD_REQUEST, {"ok": False, "error": str(exc)})
            return
        except Exception as exc:
...
<body>
    ...
    <main>
      <div class="topbar">
        ...
        <div class="controls">
          ...
          <input id="api-token" type="password" autocomplete="off" placeholder="访问令牌" aria-label="访问令牌">
          <button id="read" type="button">读取</button>
          <button id="generate" class="primary" type="button">生成</button>
          <button id="fixture" type="button">生成 fixture</button>
        </div>
      </div>
  ...
  <script>
    ...
    $("#fixture").addEventListener("click", () => loadBrief("fixture"));
    $("#market-date").addEventListener("change", () => loadBrief("read"));
    $("#api-token").value = localStorage.getItem("daily_market_brief_token") || "";
    loadBrief("read");
    ...
    async function loadBrief(action) {{
      ...
      try {{
        ...
        if (action === "read") {{
          ...
          response = await fetch(`/api/daily-market-brief?${{query.toString()}}`, {{ headers }});
        }} else {{
          response = await fetch("/api/daily-market-brief/generate", {{
            method: "POST",
            headers: {{ ...headers, "Content-Type": "application/json" }},
            body: JSON.stringify({{ market: state.market, date, fixture: action === "fixture" }})
          }});
        }}
        const data = await response.json();
        if (!data.ok) throw new Error(data.error || "处理失败");
    ...
    function statusMessage(action, data) {{
      if (data.status === "missing") return "当前市场和日期还没有简报。可以点击生成，或用 fixture 生成验收样例。";
      if (action === "fixture") return "fixture 简报已生成并保存，可用于验收页面结构和多市场共存。";
    ...
    function persistToken() {{
      const token = $("#api-token").value.trim();
      if (token) localStorage.setItem("daily_market_brief_token", token);
    }}
    function setBusy(busy) {{
      $("#read").disabled = busy;
      $("#generate").disabled = busy;
    ...
    function statusLabel(status) {{
      return {{ ok: "可用", partial: "部分可用", missing: "暂缺", provider_unavailable: "数据源暂不可用", not_available: "未提供", no_session: "无常规交易" }}[status] || status || "未知";
    }}
    function pct(value) {{
      if (value === null || value === undefined || value === "") return "-";
